<?php
// http://localhost/EC-shop/products/new_arrival.php
session_start();
?>
...
// 商品情報を新着商品から取得（LIMITとOFFSETを使用）
$stmt = $pdo->prepare("SELECT * FROM products WHERE created_at >= ? LIMIT ? OFFSET ?");
$stmt->execute([$oneMonthAgo, $productsPerPage, $offset]);
...
// ページネーションの表示
$totalProducts = $pdo->query("SELECT COUNT(*) FROM products WHERE created_at >= '$oneMonthAgo'")->fetchColumn();
